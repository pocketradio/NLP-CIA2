...
def _pairwise_acc_tfidf(processed_docs):
    """Evaluate pairwise ordering accuracy using TF-IDF features."""
    ...
    for doc in processed_docs:
        sents = doc['preprocessed_sentences'] or doc['original_sentences']
    ...
    # Build proper pairwise dataset
    X, y = [], []
    for doc in processed_docs:
        sents = doc['preprocessed_sentences'] or doc['original_sentences']
        if len(sents) < 2:
            continue
        embs = vec.transform(sents).toarray()
        n = len(sents)
        for i in range(n):
            for j in range(n):
                if i == j:
...
def evaluate_preprocessing_ablation(docs):
    """
    For each preprocessing step, evaluate pairwise ordering accuracy
    WITH vs WITHOUT that step. Uses TF-IDF cosine similarity for ordering.
    Returns dict of {step: {with: acc, without: acc}}
    """
    steps = [
        ('Lowercasing',       [],               ['--no-lowercase']),
        ('Punct Removal',     [],               ['--no-punct']),
        ('Sent Segmentation', [],               ['--no-segment']),
    ]

    results = {}

    for step_name, with_flags, without_flags in steps:
        accs = {}
        for label, flags in [('with', with_flags), ('without', without_flags)]:
            processed = preprocess_docs(docs, flags)
            acc = _pairwise_acc_tfidf(processed)
            accs[label] = acc
        results[step_name] = accs
        print(f"  {step_name}: WITH={accs['with']:.3f}, WITHOUT={accs['without']:.3f}")
