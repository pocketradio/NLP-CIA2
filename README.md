...
- **Total Documents Processed:** 100 real-world abstracts (configured via `limit` flag)
- **Train/Test Split:** 80% Training (80 abstracts), 20% Testing (20 abstracts)
...
| :--- | :---: |
| **Word2Vec (Mean Pooled)** | 52.20% |
| **Word2Vec (TF-IDF Weighted)** | 47.80% |
| **TF-IDF (Sparse Vector)** | 48.43% |
| **Contextual Token (DistilBERT)** | 66.98% |
| **Sequence Domain (SBERT)** | 64.47% |
| **Fine-Tuned Token (Siamese DistilBERT [CLS])** | **75.47%** |
...
1. **Extraction**: The pipeline reads `abstract.csv`. It filters out empty rows and targets a subset (2,000 documents).
